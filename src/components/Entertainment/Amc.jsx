...
class Amc extends Component {
    state = {
      ...
      async componentDidMount() {
        ...
        const res = await axios.get(`https://api.tdameritrade.com/v1/marketdata/chains?apikey=${process.env.REACT_APP_GITHUB_CLIENT_ID}&symbol=AMC&contractType=CALL&strikeCount=1&optionType=CALL&expMonth=NOV&toDate=${process.env.REACT_APP_DATE}&range=OTM`);
        ...
        return(
          <> 
            ...
            {!!amcData.length ? ( amcData.map(option => (
                <Card className="stockInfo" variant="outlined" style={{backgroundColor: "#6d76f7", color: '#fff'}}><i><strong>AMC Theatres</strong></i><hr></hr><i key={option.index}>
