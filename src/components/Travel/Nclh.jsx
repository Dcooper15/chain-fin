...
class Nclh extends Component {
    state = {
      ...
      async componentDidMount() {
        ...
        const res = await axios.get(`https://api.tdameritrade.com/v1/marketdata/chains?apikey=${process.env.REACT_APP_GITHUB_CLIENT_ID}&symbol=NCLH&contractType=CALL&strikeCount=1&optionType=CALL&expMonth=${process.env.REACT_APP_MONTH}&toDate=${process.env.REACT_APP_DATE}&range=OTM`);
    ...
    render() {
        ...
        return(
          <> 
            ...
            <Card className="stockInfo" variant="outlined" style={{backgroundColor: "#6d76f7", color: '#fff', borderRadius: '15px'}}><i><strong>Norwegian Cruise Line</strong></i><hr></hr><i key={option.index}>
